<?php
/**
 * Admin Action : translatablecontent/enableToTranslateAndSelectByDefault
 *
 * @category  TextMaster
...
/**
 * Class EnableToTranslateAndSelectByDefault
 *
 * Add new attribute in textmaster_translatable_content
 * if it doesn't exist and set default to true for new/existing entries
 */
class EnableToTranslateAndSelectByDefault extends AbstractAction
{
    /**
     * @var Filter
     */
    ...
    public function execute()
    {
        ...
        foreach ($attributeViewCollection as $item) {
            ...
            $isExistingItemAlreadyDefault = false;
            ...
            // 3. If line exists, we get existing id to load model and check if default has already been set to true
            if ($translatableContentRepository->getTotalCount() === 1) {
                foreach ($translatableContentRepository->getItems() as $existingItem) {
                    ...
                    $isExistingItemAlreadyDefault = $existingItem->getSelectByDefault() === true;
                }
            }
            $this->dataPersistor->set(TranslatableContent::CACHE_TAG, $data);

            $translatableContentId = $translatableContentId ?? null;
            ...
            try {
                if ($isExistingItemAlreadyDefault === false) {
        ...
        if ($successMessage === true) {
            $this->messageManager->addSuccessMessage(
                __('All attributes selected have been enabled and selected by default')
            );
        }
